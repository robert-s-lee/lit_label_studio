...
class LabelStudioJSONProcessor:
    ...
    def get_relative_image_paths(self) -> List[str]:
        ...
        relative_image_paths = [self.get_relative_image_path(annotated_example) for annotated_example in self.label_studio_json_export]
        return relative_image_paths
    
    def get_relative_image_path(self, annotated_example) -> str:
        """ paths within data_dir"""
        relative_image_path = os.path.join(self.relative_image_dir, annotated_example["data"]["img"].split("=")[-1])
        return relative_image_path
    
    def get_absolute_image_paths(self) -> List[str]:
        """ absolute paths on the machine"""
        abs_image_paths = [os.path.join(self.absolute_image_dir, annotated_example["data"]["img"].split("=")[-1]) for annotated_example in self.label_studio_json_export]
    ...
    def build_zeros_dataframe(self) -> pd.DataFrame:
        ...
        df = pd.DataFrame(np.nan*np.zeros((len(image_paths), int(len(keypoint_names)*2))), columns=pdindex, index=image_paths)
    ...
    def get_pixel_coordinates_per_image(result: Dict[str, Any]) -> Tuple[float, float]:
        ...
        value: Dict[str, Any] = result['value'] # includes the keypoint name and x,y values, relative to the image size
    ...
    def __call__(self) -> pd.DataFrame:
        ...
        for i, example in enumerate(self.label_studio_json_export): # loop over the images
            ...
            annotation = example["annotations"][0] # assume there is only one annotator per kp and image
